...
const { ActionRowBuilder, ButtonBuilder, StringSelectMenuBuilder, PermissionsBitField, TextInputBuilder, TextInputStyle } = require("discord.js");
...
module.exports = async (client, interaction, button) => {
      ...
      interaction.message.edit({
        components: [
          new ActionRowBuilder().addComponents(
            new ButtonBuilder().setStyle("Success").setLabel("Şipariş Tamamlandı").setEmoji("<:tik:971772378453540905>").setCustomId(`tamamlandı`),
            new ButtonBuilder().setLabel("Şiparişi İptal Et").setEmoji("<:cop:971772378357039124>").setCustomId(`iptal`).setStyle("Danger"),
            new ButtonBuilder().setLabel("Promosyon Kodu Kullanıldı").setEmoji("<a:star5:761479712743620608>").setCustomId(`promosyon`).setStyle("Secondary").setDisabled(true),
          )
        ],
        embeds: [
          {
  ...
  if (interaction.isStringSelectMenu()) {
    if (interaction.customId.startsWith("satinal-")) {
      ...
      let knl = await interaction.guild.channels.create({
        name: `bekliyor-${urun.name}`,
        parent_id: kategori,
        permissionOverwrites: [
          {
            id: interaction.member.id,
            allow: [PermissionsBitField.Flags.ViewChannel, PermissionsBitField.Flags.SendMessages, PermissionsBitField.Flags.AttachFiles, PermissionsBitField.Flags.ReadMessageHistory, PermissionsBitField.Flags.UseApplicationCommands, PermissionsBitField.Flags.EmbedLinks]
          },
          {
            id: mod,
            allow: [PermissionsBitField.Flags.ViewChannel, PermissionsBitField.Flags.SendMessages, PermissionsBitField.Flags.AttachFiles, PermissionsBitField.Flags.ReadMessageHistory, PermissionsBitField.Flags.UseApplicationCommands, PermissionsBitField.Flags.EmbedLinks]
      ...
      knl.send({
        components: [new ActionRowBuilder().addComponents(
          new ButtonBuilder().setLabel("Şiparişi Onayla").setEmoji("<:tickk:831064418410430474>").setCustomId(`onayla`).setStyle("Primary")
        )],
        ...
        embeds: [
          {
            ...
            fields: [
              { name: "Ürün", value: `${urun.name || "ismi bulunamadı"}`, inline: true },
    ...
    if (interaction.customId == `sil-${interaction.member.id}`) {
      if (interaction.values[0].toString() == "hepsinisil") {
        ...
        const row = new ActionRowBuilder().addComponents(new ButtonBuilder().setCustomId("sepetigör").setLabel("Sepetimi Görüntüle").setEmoji("🛒").setStyle("Secondary"))
    ...
    if (interaction.customId == `ayarlar-${interaction.member.id}`) {
      ...
      if (interaction.values[0].toString() == "duzenle") {
        ...
        sepetbilgisi.urunler.map((urun) => {
          optins.push({ label: `${urun.urunKodu || '0'} - ${urun.name || "undefined"}`, description: `${urun.desc || 'Açıklama girilmemiş'}`, value: `${urun.urunKodu || '0'}-${urun.fiyat || '0'}`, emoji: "<:cop:971772378357039124>" })
        })
        interaction.reply({
          ephemeral: true, components: [new ActionRowBuilder().addComponents(
            new StringSelectMenuBuilder().setCustomId(`sil-${interaction.member.id}`).setPlaceholder("Silmek istediğiniz ürünlerinizi seçiniz")
              .setMaxValues(1).setMinValues(1).setOptions(optins)
  ...
  if (interaction.isButton()) {
    if (interaction.customId == "promosyon") {
      const modal =
        new ModalBuilder().setCustomId("kod-kullan").setTitle("Promosyon Kodu Kullan!").setComponents(
          new ActionRowBuilder().addComponents(
            new TextInputBuilder()
              .setCustomId("girilen-kod").setLabel("Promosyon kodunuzu giriniz").setStyle(TextInputStyle.Short)
    ...
    if (interaction.customId == `tamamlandı`) {
      ...
      interaction.guild.channels.cache.get(logKanal).send({
        embeds: [new EmbedBuilder().setColor(0x00FF00).setTitle("Sipariş Onaylandı")
          .setDescription(
    ...
    if (interaction.customId == `iptaledildi`) {
      ...
      let sipData = await islemler.findOne({ channelId: interaction.message.channelId });
      interaction.guild.channels.cache.get(logKanal).send({
        embeds: [new EmbedBuilder().setColor(0xFF0000).setTitle("Sipariş İptal Edildi")
          .setDescription(
            `Müşteri Kullanıcı : <@${sipData.memberId}>(\`${sipData.memberId}\`)
    ...
    if (interaction.customId == `iptal`) {
      ...
      interaction.reply({
        ephemeral: true, content: "Siparişi iptal etmek istediğinize emin misiniz? Bu işlem geri alınamaz", components: [
      ...
      interaction.message.edit({
        components: [
          new ActionRowBuilder().addComponents(
            new ButtonBuilder().setStyle("Success").setLabel("Şipariş Tamamlandı").setEmoji("<:tik:971772378453540905>").setCustomId(`tamamlandı`),
            new ButtonBuilder().setLabel("Şiparişi İptal Et").setEmoji("<:cop:971772378357039124>").setCustomId(`iptal`).setStyle("Danger"),
            new ButtonBuilder().setLabel("Promosyon Kodu Kullan").setEmoji("<a:star5:761479712743620608>").setCustomId(`promosyon`).setStyle("Secondary"),
          )
        ]
      })
    ...
    if (interaction.customId == "sepetigör") {
      ...
      **<:itemsatis:992006158267207680> İtemsatış ile öderseniz: ${(total + (total * 7 / 100) + (total * 5 / 100)).toString().slice(0, 4)}TL**`)
      ...
      const row = new ActionRowBuilder().addComponents(
        new StringSelectMenuBuilder().setCustomId(`ayarlar-${interaction.member.id}`).setPlaceholder("Sepet Ayarları")
      ...
      if (_data) {
        if (tur == "al") {
          const row = new ActionRowBuilder().addComponents(
            new StringSelectMenuBuilder().setCustomId(`satinal-${_data.urunKodu}`).setPlaceholder("Ödeme Yönetmini Seçiniz")
              .setMaxValues(1).setMinValues(1).setOptions([
          ...
          interaction.reply({
            embeds: [{ title: "Ödeme Yöntemi Seç", description: "Satın Alma için kullanıcağınız ödeme yönetmini seçin" }],
          ...
          if (bilgiler) return interaction.reply({
            ephemeral: true, components: [new ActionRowBuilder().addComponents(
              new ButtonBuilder().setLabel("Sepetimi görünütle").setCustomId("sepetigör").setEmoji("🛒").setStyle("Secondary")
